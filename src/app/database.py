...
class MongoManager:
    ...
    # MESSAGES
    async def get_chat_message_records(self, chat_id: int, topic_id: int, offset: int = 0, sort=None) -> list[MessageRecord]:
    ...
    # CHATS
    def sync_add_chat(self, chat_info: ChatInfo):
    ...
    async def add_chat(self, chat_info: ChatInfo):
        # assert isinstance(chat_info, ChatInfo)  # todo: type check
        self.logger.info(f"chat created: {chat_info}")
        self.chat_info_collection.insert_one(chat_info)
    ...
    async def update_chat_info(self, chat_info: ChatInfo):
        # assert isinstance(chat_info, ChatInfo)  # todo: type check
        self.chat_info_collection.replace_one({"_id": chat_info["_id"]}, chat_info)

    # TOPICS
    async def add_topic(self, topic_info: TopicInfo, chat_id: int):
    ...
    async def update_topic_info(self, topic_info: TopicInfo, chat_id: int):
        # assert isinstance(topic_info, TopicInfo)  # todo: type check
        assert isinstance(chat_id, int)
        col = self.topics_db.get_collection(str(chat_id))
